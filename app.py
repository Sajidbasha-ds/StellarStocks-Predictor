        ...
        if df is not None and not df.empty:
            ...
            if peer:
                ...
                if peer_df is not None and not peer_df.empty:
                    left_df = df[['Date', 'Open', 'High', 'Low', 'Close', 'Volume']].copy()
                    left_df.columns = ['Date'] + [f'{ticker} {col}' for col in ['Open', 'High', 'Low', 'Close', 'Volume']]
                    right_df = peer_df[['Date', 'Open', 'High', 'Low', 'Close', 'Volume']].copy()
                    right_df.columns = ['Date'] + [f'{peer} {col}' for col in ['Open', 'High', 'Low', 'Close', 'Volume']]
                    merged = pd.merge(left_df, right_df, on='Date', how='outer')
                    st.caption(f"Table: Open, High, Low, Close, and Volume for {ticker} and {peer}.")
                    st.dataframe(merged, use_container_width=True)
                else:
                    st.caption(f"Table: Open, High, Low, Close, and Volume for {ticker}.")
                    st.dataframe(df[['Date', 'Open', 'High', 'Low', 'Close', 'Volume']].rename(
            ...
            if peer and peer_df is not None and not peer_df.empty:
                ax1.plot(peer_df['Date'], peer_df['Close'], label=f"{peer} Close", color="#2196f3", linestyle="--", linewidth=2.1)
            ax1.set_title(f"{ticker}" + (f" vs {peer}" if peer else "") + " Closing Price")
            ax1.set_xlabel("Date")
...
if page == "Prediction":
    ...
    if st.button("Run Prediction"):
        ...
        if df is not None and len(df) > 30:
            ...
            display_company_info(ticker, branch)
            # ------- ARTICLES REMOVED HERE: no news panel -------
            # ML - Classic Regression Models
